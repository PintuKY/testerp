...
class TransactionUtil extends Util
{
    ...
    public function createSellTransaction($contact, $business_id, $input, $invoice_total, $user_id, $uf_data = true)
    {
        ...
        if ($input['status'] == AppConstant::FINAL) {
            $rp_earned = $this->calculateRewardPoints($business_id, $final_total);
        } elseif ($input['status'] == AppConstant::PROCESSING) {
            $rp_earned = $this->calculateRewardPoints($business_id, $final_total);
        } elseif ($input['status'] == AppConstant::COMPLETED) {
            $rp_earned = $this->calculateRewardPoints($business_id, $final_total);
        } else {
            $rp_earned = 0;
        }
     ...
     */
    public function createOrUpdateSellLines($input, $transaction, $products, $location_id, /*$number_of_days, $time_slot, $has_purchase_due,*/ $return_deleted = false, $status_before = null, $extra_line_parameters = [], $uf_data = true,)
    {
    ...
    public function createOrUpdateSupplierProductSellLines($transaction, $products, $location_id, $return_deleted = false, $status_before = null, $extra_line_parameters = [], $uf_data = true)
    {
        $lines_formatted = [];
        $modifiers_array = [];
        $edit_ids = [0];
        ...
        foreach ($products as $product) {
            ...
            if (!empty($product['transaction_sell_lines_id'])) {
                ...
                $edit_id_temp = $this->editSupplierProductSellLine($product,$status_before, $location_id,  $multiplier, $uf_data);
                $edit_ids = array_merge($edit_ids, $edit_id_temp);
                ...
                $lines_formatted[] = new SupplierTransactionSellLine($line);

                $sell_line_warranties[] = !empty($product['warranty_id']) ? $product['warranty_id'] : 0;

                //Update purchase order line quantity received
                $this->updateSupplierProductSalesOrderLine($line['so_line_id'], $line['quantity'], 0);
            }
        }
        if (!is_object($transaction)) {
            $transaction = SupplierTransaction::findOrFail($transaction);
        }

        //Delete the products removed and increment product stock.
        $deleted_lines = [];
        if (!empty($edit_ids)) {
            $deleted_lines = SupplierTransactionSellLine::where('supplier_transaction_id', $transaction->id)
                    ->whereNotIn('id', $edit_ids)
                    ->select('id')->get()->toArray();
            // $combo_delete_lines = SupplierTransactionSellLine::whereIn('parent_sell_line_id', $deleted_lines)->where('children_type', 'combo')->select('id')->get()->toArray();
            // $deleted_lines = array_merge($deleted_lines, $combo_delete_lines);
            ...
            if ($this->isModuleEnabled('modifiers')) {
                foreach ($lines_formatted as $key => $value) {
                    if (!empty($modifiers_array[$key])) {
                        foreach ($modifiers_array[$key] as $modifier) {
                            $modifier['parent_sell_line_id'] = $value->id;
                            $modifiers_formatted[] = new SupplierTransactionSellLine($modifier);
                        }
                    }
                }
            }
    ...
    private function updateSupplierProductSalesOrderLine($so_line_id, $new_qty, $old_qty = 0)
    {
        $diff = $new_qty - $old_qty;
        if (!empty($so_line_id) && !empty($diff)) {
            $so_line = SUpplierTransactionSellLine::find($so_line_id);
            $so_line->so_quantity_invoiced += ($diff);
            $so_line->save();
        }
    }
     ...
     * @return boolean
     */
    public function editSellLine($product, $product_line, $variationId, $location_id, $status_before, $multiplier = 1, $uf_data = true)
    {

        $productLine = $product_line[$product['product_id']];
        ...
        $uf_unit_price = $uf_data ? $this->num_uf($productLine['unit_price']) : $productLine['unit_price'];
        ...
        $sell_line->fill(['product_id' => $product['product_id'],
            'variation_id' => $variationId,
            'quantity' => $uf_data ? $this->num_uf($productLine['quantity']) * $multiplier : $productLine['quantity'] * $multiplier,
            'unit_price_before_discount' => $unit_price_before_discount,
            'unit_price' => $unit_price,
            'line_discount_type' => !empty($product['line_discount_type']) ? $product['line_discount_type'] : null,
            'line_discount_amount' => $line_discount_amount,
            ...
            'tax_id' => $product['tax_id'],
            'unit_price_inc_tax' => $uf_data ? $this->num_uf($productLine['unit_price_inc_tax']) / $multiplier : $productLine['unit_price_inc_tax'] / $multiplier,
            'sell_line_note' => !empty($product['sell_line_note']) ? $product['sell_line_note'] : '',
            'sub_unit_id' => !empty($product['sub_unit_id']) ? $product['sub_unit_id'] : null,
            'res_service_staff_id' => !empty($product['res_service_staff_id']) ? $product['res_service_staff_id'] : null
        ]);
    ...
    public function editSupplierProductSellLine($product,$status_before, $location_id,  $multiplier = 1, $uf_data = true)
    {
        //Get the old order quantity
        $sell_line = SupplierTransactionSellLine::with(['product'])
        ->find($product['transaction_sell_lines_id']);
        ...
        $this->updateSupplierProductSalesOrderLine($sell_line->so_line_id, $sell_line->quantity, $old_qty);
        return $edit_ids;
    }
    ...
    public function deleteSupplierProductSellLines($transaction_line_ids, $location_id, $adjust_qty = true)
    {

        if (!empty($transaction_line_ids)) {

            $sell_lines = SupplierTransactionSellLine::whereIn('id', $transaction_line_ids)
                ->get();
            //Adjust quanity

            foreach ($sell_lines as $line) {
                if ($adjust_qty) {
                    $this->adjustSupplierProductQuantity($location_id, $line->product_id, $line->variation_id, $line->quantity);
                }

                //Update purchase order line quantity received
                $this->updateSupplierProductSalesOrderLine($line->so_line_id, 0, $line->quantity);
            }
    ...
    private function adjustSupplierProductQuantity($location_id, $product_id, $variation_id, $increment_qty)
    {
        if ($increment_qty != 0) {
            // $enable_stock = Product::find($product_id)->enable_stock;
            ...
            // if ($enable_stock == 1) {
                //Adjust Quantity in variations location table
                SupplierProductLocationDetails::where('variation_id', $variation_id)
                    ->where('product_id', $product_id)
                    ->where('location_id', $location_id)
                    ->increment('qty_available', $increment_qty);
            // }
            ...
            if (!($line->modifiers->isEmpty())) {
                foreach ($line->modifiers as $modifier_line) {
                    ...
                    $modifier_line_array = [
                        ...
                        'total_item_value' => $modifier_line->total_item_value,
                        'tran_sell_var_value' => ($modifier_line->transactionSellLinesVariants->isNotEmpty()) ? $modifier_line->transactionSellLinesVariants[0]->value : '0',
                        //Field for 2nd column
    ...
    protected function _receiptDetailsSellReturnLines($lines, $il, $business_details)
    {
        ...
        foreach ($lines as $line) {
            ...
            $line_array = [
                ...
                'quantity_uf' => $line->quantity,
                'total_item_value' => $line->total_item_value,
                'units' => $unit_name,
    ...
    public function mapSupplierProductPurchaseSell($business, $transaction_lines, $mapping_type = 'purchase', $check_expiry = true, $purchase_line_id = null)
    {
        if (empty($transaction_lines)) {
            return false;
        }
...
//Get sell lines used for the transaction.
    ...
    )
    {
        ...
        if (in_array('sell', $transaction_types)) {
            $query->addSelect(
                DB::raw("SUM(IF(transactions.type='sell' AND (transactions.status=$final OR transactions.status=$processing OR  transactions.status=$completed), IF(discount_type = 'percentage', COALESCE(discount_amount, 0)*total_before_tax/100, COALESCE(discount_amount, 0)), 0)) as total_sell_discount"),
                DB::raw("SUM(IF(transactions.type='sell' AND (transactions.status=$final OR transactions.status=$processing OR  transactions.status=$completed), rp_redeemed_amount, 0)) as total_reward_amount"),
                DB::raw("SUM(IF(transactions.type='sell' AND (transactions.status=$final OR transactions.status=$processing OR  transactions.status=$completed), round_off_amount, 0)) as total_sell_round_off")
            );
        }
    ...
    public function getGrossProfit($business_id, $start_date = null, $end_date = null, $location_id = null, $user_id = null)
    {
        $query = TransactionSellLine
            ...
            ->where('sale.status', AppConstant::FINAL)
            ->orWhere('sale.status', AppConstant::PROCESSING)
            ->orWhere('sale.status', AppConstant::COMPLETED)
    ...
    public function deleteSale($business_id, $transaction_id)
    {
        ...
        if (!empty($transaction)) {
            ...
            if ($transaction->status == 'draft') {
                ...
                $transaction->status = AppConstant::PAYMENT_PENDING;
                $business_data = ['id' => $business_id,
                    'accounting_method' => $business->accounting_method,
                    'location_id' => $transaction->location_id
                ];

                $this->adjustMappingPurchaseSell(AppConstant::FINAL, $transaction, $business_data, $deleted_sell_lines_ids);
    ...
    public function getLedgerDetails($contact_id, $start, $end)
    {
        ...
        $previous_transaction_sums = $this->__transactionQuery($contact_id, $start)
            ->select(
                ...
                DB::raw("SUM(IF(type = 'sell' AND (status=$final OR status=$processing OR  status=$completed), final_total, 0)) as total_invoice"),
    ...
    public function addSellReturn($request, $input, $business_id, $user_id, $uf_number = true)
    {
        ...
        $sell_return_data = [
            ...
            'discount_amount' => $uf_number ? $this->num_uf($sell->discount_amount) : $sell->discount_amount,
            'tax_id' => $sell->tax_id,
        ...
        if (empty($sell_return)) {
            ...
            $sell_return_data['status'] = $sell->status;
    ...
    public function getUserTotalSales($business_id, $user_id, $start_date, $end_date)
    {
        $totals = Transaction::where('business_id', $business_id)
            ...
            ->where('type', 'sell')
            ->where('status', AppConstant::FINAL)
            ->orWhere('status', AppConstant::COMPLETED)
            ->orWhere('status', AppConstant::PROCESSING)
