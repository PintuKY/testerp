$(document).ready(function() {
    ...
    $('#supplier_id').select2({
        ...
        templateResult: function(data) {
            ...
            var html = data.text + ' - ' + data.business_name + ' (' + data.supplier_id + ')';
            return html;
        },
        language: {
            noResults: function() {
    ...
    $('.supplier_modal').on('hidden.bs.modal', function() {
        $('form#quick_add_supplier')
            ...
            .removeAttr('disabled');
        $('form#quick_add_supplier')[0].reset();
    });
    ...
    $(document).on('change','#purchase_list_filter_location_id, \#purchase_list_filter_supplier_id, #purchase_list_filter_payment_status,\#purchase_list_filter_status',
        function() {
...
$(document).on('change', '#supplier_id', function(){
    if ($('#purchase_order_ids').length) {
        ...
        $.ajax({
            url: '/get-purchase-orders/' + supplier_id,
            dataType: 'json',
