...
@section('content')
    ...
    <section class="content-header">
        <h1>{{$title}} <small>(@if($transaction->type == 'sales_order')
                ...
                @component('components.widget', ['class' => 'box-solid'])
                    ...
                    @if(in_array('types_of_service', $enabled_modules) && !empty($transaction->types_of_service))
                        <div class="col-md-4 col-sm-6">
                            <div class="form-group">
                                ...
                                <small><p
                                        class="help-block @if(empty($transaction->selling_price_group_id)) hide @endif"
                    ...
                    <div class="@if(!empty($commission_agent)) col-sm-3 @else col-sm-4 @endif">
                        <div class="form-group">
                            ...
                            <small
                                class="text-danger @if(empty($customer_due)) hide @endif contact_due_text"><strong>@lang('account.customer_due')
                    ...
                    <div class="@if(!empty($commission_agent)) col-sm-3 @else col-sm-4 @endif">
                        <div class="form-group">
							...
							<span class="input-group-addon">
                                ...
                                {!! Form::text('transaction_date', $transaction->transaction_date, ['class' => 'form-control', 'required']); !!}
                            </div>
                        </div>
                    </div>
                    @php
                                ...
                                <table
                                    ...
                                    <tr>
                                        <th class="text-center">
                                            @lang('sale.product') ({{$product_names[$key]}})
                                        </th>
                                        ...
                                        @if($sell_line->product->type === "single")
                                            @if($sell_line->product_id == $productId)
                                                <tr class="product_row" data-row_index="{{ $loop->index}}"
                                                    ...
                                                    <td>Single</td>
                                                    <td>{{$sell_line->total_item_value}}</td>
                                                </tr>
                                            @endif
                                       @endif


                                        @if(!($sell_line->transactionSellLinesVariants->isEmpty()) && $sell_line->product->type === "variable")
     ...
     "checked" : '' }}>
                ...
                <span class="input-group-btn"><button type="button"
                            ...
                            class="fa fa-minus text-danger"></i></button></span>
                                                ...
                                                <div
                                                    class="col-sm-6 @if(!auth()->user()->can('edit_product_price_from_sale_screen')) hide @endif">
                                                    <div class="form-group">
